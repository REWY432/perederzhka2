...
import { X, AlertTriangle, Check, Syringe } from 'lucide-react';
...
const BookingForm: React.FC<Props> = ({ initialData, allBookings, onClose, onSave }) => {
  ...
  const inputClass = "w-full p-2.5 bg-white/50 dark:bg-black/40 border border-white/30 dark:border-white/10 rounded-lg focus:ring-2 focus:ring-teal-500/50 outline-none backdrop-blur-sm transition-all text-gray-800 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/80 dark:bg-gray-900/90 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/40 dark:border-white/10 w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar transition-colors duration-300">
        
        {/* Header */}
        <div className="flex justify-between items-center p-6 border-b border-black/5 dark:border-white/10 bg-white/30 dark:bg-black/20">
          <div className="flex items-center gap-3">
             <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-teal-700 to-blue-700 dark:from-teal-400 dark:to-blue-400">
                {initialData ? 'Редактировать' : 'Новое заселение'}
             </h2>
          </div>
          <button onClick={onClose} className="p-2 bg-white/50 dark:bg-white/10 rounded-full hover:bg-white/80 dark:hover:bg-white/20 transition-colors text-gray-600 dark:text-gray-300">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          {/* Financials */}
          <div className="bg-white/40 dark:bg-black/40 p-5 rounded-2xl border border-white/30 dark:border-white/10 space-y-4 shadow-inner">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                  <label className={labelClass}>Цена в день (₽)</label>
                  <input
                    type="number"
                    className={inputClass}
                ...
                <div>
                  <label className={labelClass}>Памперсы/Проч (₽)</label>
                  <input
                    type="number"
                    className={inputClass}
                ...
                <div>
                  <label className={labelClass}>Ущерб (₽)</label>
                  <input
                    type="number"
                    className={inputClass}
             ...
             {/* Cost Breakdown */}
             <div className="pt-4 border-t border-gray-200/50 dark:border-white/10 mt-2 space-y-2">
                <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                    <span>Проживание ({days} дн. x {formData.pricePerDay}₽):</span>
                    <span className="font-medium">{(days * formData.pricePerDay).toLocaleString()} ₽</span>
                </div>
                {(formData.diaperCost > 0) && (
                    <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                        <span>Доп. расходы:</span>
                        <span className="font-medium">{formData.diaperCost.toLocaleString()} ₽</span>
                    </div>
                )}
                {(formData.damageCost > 0) && (
                    <div className="flex justify-between text-sm text-red-600 dark:text-red-400">
                        <span>Ущерб:</span>
                        <span className="font-medium">{formData.damageCost.toLocaleString()} ₽</span>
                    </div>
                )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Checklist */}
              <div>
                <label className={labelClass}>Привезено с собой</label>
                            ...
                            <div 
                                ...
                                className={`cursor-pointer p-2 rounded-lg border flex items-center gap-2 transition-all text-xs font-bold ${
                                    checked 
                                        ...
                                        : 'bg-white/40 dark:bg-white/5 border-white/40 dark:border-white/10 text-gray-500 dark:text-gray-400'
                                }`}
                            >
